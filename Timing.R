...
#Time functions
system.time(energy(s,network=data,population=pop)) #Energy takes 4.7 sec... :P

system.time(neighbour(s,N,grannar=1)) #Neighbour takes 0 sec

system.time(P(15011,1/10)) #P takes 0 sec

system.time(sa(network=data,population=pop,p=2,grannar=1,tmax=1)) #SA takes engergy*2 sec, i.e. 9.44 sec